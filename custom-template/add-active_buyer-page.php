<?php
/*
 Display Template Name: Add Active Buyer
*/
 if (!is_user_logged_in() ) {
 wp_redirect ( home_url("/buyer-register") );  
}
get_header();
?><div class="container">
   <div class="content">
   <?php $user_id = get_current_user_id(); ?>
   <?php if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == "active-buyer") { ?>
      <?php
         if(isset($_POST['submit'])){
          $title=$_POST['title'];
         $body=$_POST['body'];
         $buyer_category=$_POST['buyer-category'];
         $require_content = $_POST['my_requirement'];
         ...
         add_post_meta($post_id, 'my_requirement', $require_content);
         ...
         $cat_ids = array($buyer_category);
         
         $cat_ids = array_map( 'intval', $cat_ids );
         $cat_ids = array_unique( $cat_ids );
         
         $term_taxonomy_ids = wp_set_object_terms($post_id, $cat_ids, 'buyer-category' );
         ...
         ?>
 <h1>Add Active Buyer Listing</h1>
 <?php if(!is_user_logged_in()) {

               echo '<div class="must-be-logged-in">';
               echo '<h4 class="center marB20">' . __('You must be logged in to view this page.', 'contempo') . '</h4>';
                    echo '<p class="center login-register-btn marB0"><a class="btn login-register" href="'.site_url().'/buyer-register/">Login/Register</a></p>';
                echo '</div>';
                
            }  elseif(in_array('buyer', (array) $current_user->roles)) { 

                $item_count = $wpdb->get_var( "SELECT count(*) FROM $wpdb->posts WHERE post_status = 'publish' AND post_type = 'active-buyer' AND post_author = $userdata->ID" );
       ...
       <form name="frmContact" action="" method="post" enctype="multipart/form-data" onsubmit="return validateContactForm()">
        <div class="row">
          ...
          <div class="from-group col-md-6">
          <textarea class="from-control input-field" id="require_content" placeholder="Extra Details*" name="my_requirement"></textarea>
            ...
            <div class="from-group col-md-6">
                  ...
                  <?php
                     //ref. https://codex.wordpress.org/Function_Reference/get_terms
                      $terms = get_terms("buyer-category",'order_by=count&hide_empty=0');
                     if ( !empty( $terms ) && !is_wp_error( $terms ) ){
                         echo "<select name='buyer-category' class='from-control'>";
                         echo "<option selected='selected'> Buyer-Category </option>";
    ...
    <div class="from-group col-md-6">
          ...
          <input id="location" type="text" class="from-control input-field" placeholder="Location*" name="location" />
           <span id="userlocation" class="info"></span>
            ...
            <div class="from-group col-md-6">
                <input id="investment_from" type="text" class="from-control input-field" placeholder="Investment Range From*" name="investment_from" pattern="\d+(\.\d{2})?" />
                 <span id="userinvestment_from" class="info"></span>
            </div>
            <div class="from-group col-md-6">
            <input id="investment_to" type="text" class="from-control input-field" placeholder="Investment Range To*" name="investment_to" pattern="\d+(\.\d{2})?"/>
...
<input type="hidden" name="action" value="active-buyer" />
            </div>
...
<script type="text/javascript">
 function validateContactForm() {
            ...
            if (Investment_Range_From == "") {
                $("#userinvestment_from").html("Please Enter Investment From.");
